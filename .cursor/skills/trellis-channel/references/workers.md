...
`channel interrupt` is the cooperative redirect: it appends
`interrupt_requested` and, when an active worker turn is interrupted,
`interrupted` events with reason `"user"`. Where the adapter supports it, the
supervisor also issues a provider-level turn interrupt with a replacement
instruction. Use it when the worker should drop its current turn and act on new
input immediately, without losing its session.
...
The appended events use the official `interrupt_requested` / `interrupted`
