...
- **Dispatcher wait pattern**: use `--kind done` / `--kind turn_finished` (trellis-emitted system events), not custom completion text. The v0.6.0 channel CLI has no `--tag` flag; `trellis channel interrupt` is a dedicated command, and completion / turn routing should rely on event kinds. See `references/command-reference.md` "tag-vs-kind".
