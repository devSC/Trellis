...
4. `guru_chain=light` 只描述精简产物形态；Lite 执行仍是 compact intake→host-inline→deterministic_final，不继承 Full Gate。
...
Full 与 Trellis 原版同构（dispatch 协议、commit 批量确认流程、finish-work 收尾不变），仅口径替换为 H5：
