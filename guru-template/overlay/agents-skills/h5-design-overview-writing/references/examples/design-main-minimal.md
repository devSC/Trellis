...
> 本文件是 **成稿形态示例**（取材自参考项目 `/Users/devSC/Documents/MyProject/next.js/examples/blog` 的真实内容单元，按 App Router 生产形态改编），只演示各章形态与粒度，**不是规则来源**；章节合同以概要 L1（`.trellis/spec/harness/overview/overview-structure-single-source.md`）为准。
> 取材真实单元：blog 示例的「文章 + 标签 + RSS」内容单元——`pages/posts/markdown.md`、`pages/posts/pages.md`（frontmatter `title/date/description/tag/author`）、`pages/tags/[tag].mdx`（按 tag 列文章，用 `useRouter().query` 取参）、`scripts/gen-rss.js`（`fs.readdir(pages/posts)` + `gray-matter` + `rss` 生成 `public/feed.xml`）、`package.json` build=`node ./scripts/gen-rss.js && next build`。这些是 **`[示例实证]`**；App Router 段/RSC/server-action/strict 等是 **`[生产级补充]`**（示例为 Pages Router + Nextra，`tsconfig.json` `strict:false`/`target:es5`，不沿用）。
...
- 路由模式：App Router（SLOT-13，生产基线）。`[生产级补充]` 示例为 Pages Router（`pages/posts/*.md`、`pages/tags/[tag].mdx`），生产以 `app/` 段为准，不沿用。
- 内容源：本地 MDX（SLOT-05）。`[示例实证]` 内容模型见 `examples/blog/pages/posts/markdown.md` frontmatter；生产将「读 MDX + 解析 frontmatter」封装进 data-access（示例逻辑散落在 `scripts/gen-rss.js`，属 `[生产级补充]`）。
...
- TS：`strict: true`（生产硬底线）。`[生产级补充]` 示例 `tsconfig.json` 为 `strict:false`/`target:es5`/`typescript ^4.7.4`，不沿用。

### 1.3 显式假设

| 假设 | 依据 | 影响范围 | 验证时点 |
|------|------|---------|---------|
| 标签为字符串单值（示例 frontmatter `tag: web development`） | `examples/blog/pages/posts/markdown.md` frontmatter `tag` 单字符串 | tag 路由段与筛选粒度（单标签）；多标签需扩 domain-type | 详细设计前向需求确认是否需多标签 |
...
> `[示例实证]` 示例用 `pages/tags/[tag].mdx` + `useRouter().query.tag` 在客户端取参（Pages Router 写法）；`[生产级补充]` 生产用动态段 `params.tag` 在 server-component 取参，避免客户端取数。
...
> `[示例实证]` 示例在构建期由 `scripts/gen-rss.js`（`fs.readdir` + `gray-matter` + `rss`）写 `public/feed.xml`；`[生产级补充]` 生产可改为 App Router route handler `app/feed.xml/route.ts` 经 data-access 共用取数逻辑（构建期脚本与运行时 endpoint 二选一，本稿取 route handler 形态作 server-action 类承接示例）。
...
| BHV-003/004 | server-component / UNIT-tag-list-server-component | 按 tag 取数编排 + 空集 `notFound()` 收口是服务端渲染职责 | 不属于 client-component（示例客户端取参为 Pages 写法，生产禁 client 直取内容源）；不属于 data-access（data-access 只过滤取数，不调 `notFound()`/不编排渲染） | 是：标签页主渲染单元 |
...
graph TD
    ...
    subgraph external["external"]
        MDX["本地 MDX 内容源 (pages/posts/*.md 形态)"]
    end
...
|------------|----------------|-----------|----------|-----------|--------------------------|------------------|
| TD-01 | 内容源形态 | 本地 MDX / Headless CMS | 本地 MDX | 单作者小站、内容随仓库版本管理，无需多编辑者后台；对齐示例内容模型（`pages/posts/*.md` frontmatter） | posts-data-access（封装 MDX 读取 + frontmatter 校验，不写调用参数） | N/A（无 PII/认证/三方域名） |
...
| TD-03 | RSS 产出形态 | 构建期脚本（示例 gen-rss.js）/ 运行时 route handler | route handler | 与页面共用 data-access 取数，避免逻辑两份；endpoint 形态便于按需重生成 | feed-route-handler（不写响应头/RSS 字段正文） | N/A |
...
| G9 golden-path 自检 | 满足 | strict:true / server-first / secret 隔离 / route 段约定 / metadata 标准化 / 样式隔离均合锁定项；`[示例实证]`/`[生产级补充]` 标签使用正确 |
