...
- 全稿完成 → 回填全部时序图占位 → G1~G9 自检 → 提示送审（加载 `h5-design-overview-review` 过概要 Gate；由 review worker 用 `record-review overview` 记录当前 digest 的 clean/findings 证据，区别于 `trellis-check` 代码质检）。
