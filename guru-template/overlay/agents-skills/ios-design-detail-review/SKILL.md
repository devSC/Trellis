...
- **编号纪律**（机器追溯依据，详见 index §4.1）：行为 `### BHV-NNN <短名>`（prd 标题，正则 `^#{2,5}\s+BHV-\d+`，按「用户操作 / 系统反应 / 失败路径 / 生命周期」枚举，创建后不复用、不重排、删除留洞）；设计单元 `### UNIT-<slug>`（详细标题，正则 `^#{2,5}\s+UNIT-[a-z0-9][a-z0-9-]*`，slug 建议带 doc_type 后缀自证归属：`UNIT-home-viewmodel` / `UNIT-story-usecase` / `UNIT-story-repository` / `UNIT-story-model` / `UNIT-home-view` / `UNIT-app-coordinator` / `UNIT-database-external`）；下游引用（归属表、单元、测试映射、实现切片）一律写裸 token，`guru_gate.py trace-matrix` 据此生成 行为×归属×单元×测试×切片 矩阵，断链被 Gate 拦截。
