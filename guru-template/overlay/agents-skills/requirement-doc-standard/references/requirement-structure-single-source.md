...
- **负向 / 排除场景（P1，§4.3 invariant 来源）**：`REQ-UC` 在适用处须显式声明负向语义——must-not 行为、保留-排除规则、成功路径排除（如"远端权威未返回的项不应出现在可见集"）。这些负向需求是详细设计**不变量矩阵**（detail-structure §9 / slice packet `invariants[]`）的需求级来源；正向行为描述不能替代负向/排除约束的声明。
