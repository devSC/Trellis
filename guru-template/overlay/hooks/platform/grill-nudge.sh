#!/usr/bin/env bash
# PostToolUse(Write|Edit|MultiEdit)：prd/归属表草稿"成形"后自动提示加载 __GRILL_SKILL__ 拷问。
# __GRILL_SKILL__ 由 apply.sh 按平台 sed 替换（flutter→client-grill、go→go-design-grill、ios→ios-design-grill、h5→h5-design-grill）。
...
if python3 "$GATE" "$KIND" "$TASK_DIR" >/dev/null 2>&1; then
  touch "$MARK"
  echo "📋 ${ART}.md 已通过结构检查（${KIND} Gate 口径）。按 workflow：送审前先加载 __GRILL_SKILL__ 拷问——对照 golden-path/项目约定/既有 BHV 磨术语、压测边界场景，决策当场固化进产物；拷问后再走对应 review 提交人工 Gate。（本提示仅一次；删除 $MARK 可重新触发）" >&2
  exit 2
fi
exit 0
