...
`adversarial_enabled: false` 只关闭 `guru_supervise.py --adversarial ...` 的 opposite-provider worker 启动，用来临时缩短任务耗时。它不会伪造 clean 证据：requirements 会记录为 `requirements_review.status=deferred` 并在确认时警告；overview/detail 仍需要当前 Gate 要求的 review evidence，否则 `guru_gate.py status/check` 仍会提示缺口。
