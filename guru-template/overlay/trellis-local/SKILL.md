...
- **Skills**: 8 个 `client-*`（`.agents/skills/`）+ shared `design-grill`：design-overview/detail 的 writing+review、flutter-implementation 的 writing+review、small-iteration-dev（入口分流）、**design-grill**（Gate 前拷问，grill-with-docs 适配版：brainstorm 探索 → grill 拷问 → review 判定 三拍；平台旧名仅作 wrapper）
