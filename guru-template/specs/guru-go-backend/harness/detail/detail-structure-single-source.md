...
**破坏性编辑保护（删除/压缩/替换）**：详细章节被重写时，接口事实可删除或替换，但仍有效的合同义务不得消失。删除前必须列 deletion ledger：`deleted_category` / `reason` / `replacement_location` / `removes_contract_obligation` / `reviewer_decision`。L1 章节骨架、UNIT/BHV 承接、行为定义、输入输出错误合同、状态/事务 owner、失败收口、测试映射、不得补造清单必须保留、迁移到命名替代位置，或以 `N/A：<理由>` 显式声明；不得用 endpoint/interface 映射表替代行为合同。
