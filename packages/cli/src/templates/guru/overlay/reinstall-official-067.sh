...
GIT_INDEX_DIGEST_KIND = "git-effective-index-logical-record-v3"
...
def log(message: str) -> None:
    print(f"[guru-reinstall-067] {safe_text(message)}", flush=True)
...
def live_git_tree_records(template_root: Path) -> list[tuple[str, ...]]:
    records: list[tuple[str, ...]] = []
    for row in path_records(template_root, ignore_noise=True):
        path = str(row["path"])
        entry_type = str(row["type"])
        if entry_type == "directory":
            records.append((path, entry_type))
        elif entry_type == "file":
            git_mode = "100755" if int(row["mode"]) & 0o111 else "100644"
            records.append((path, entry_type, git_mode, str(row["sha256"])))
        elif entry_type == "symlink":
            target = os.fsencode(str(row["target"]))
            records.append((path, entry_type, sha256_bytes(target)))
        else:
            records.append((path, entry_type))
    return sorted(records)


def head_git_tree_records(
    checkout_root: Path,
    ...
    prefix = "" if relative_template == "." else f"{relative_template}/"
    entries: list[tuple[str, str, str]] = []
    blob_ids: list[str] = []
    found_template_root = relative_template == "."
    for raw_entry in tree_result.stdout.split(b"\0"):
        ...
        try:
            ...
            return None
        full_path = os.fsdecode(raw_path)
        mode = raw_mode.decode("ascii", errors="strict")
        entry_type = raw_type.decode("ascii", errors="strict")
        object_id = raw_object_id.decode("ascii", errors="strict")
        if full_path == relative_template:
            found_template_root = entry_type == "tree"
            continue
        if relative_template.startswith(f"{full_path}/") and entry_type == "tree":
            continue
        if prefix and not full_path.startswith(prefix):
            return None
        relative = full_path[len(prefix) :] if prefix else full_path
        parts = Path(relative).parts
        if entry_type == "tree" and parts and parts[-1] == "__pycache__":
            continue
        ...
        if entry_type == "blob":
            ...
            return None
    if not found_template_root:
        return None

    # Read raw HEAD blobs, not the index or checkout. The only deliberately
    # excluded noise matches path_records(): __pycache__, .DS_Store, and *.pyc.
    blob_result = subprocess.run(
        ["git", "-C", str(checkout_root), "cat-file", "--batch"],
    ...
    for requested_id in blob_ids:
        header_end = blob_result.stdout.find(b"\n", offset)
        if header_end < 0:
            return None
        header = blob_result.stdout[offset:header_end].split(b" ")
        if len(header) != 3 or header[1] != b"blob":
            return None
        try:
            returned_id = header[0].decode("ascii", errors="strict")
            size = int(header[2])
        except (UnicodeDecodeError, ValueError):
            return None
        content_start = header_end + 1
        content_end = content_start + size
        if returned_id != requested_id or content_end >= len(blob_result.stdout):
            return None
        if blob_result.stdout[content_end : content_end + 1] != b"\n":
            return None
        blob_digests[requested_id] = sha256_bytes(
            blob_result.stdout[content_start:content_end]
        )
        offset = content_end + 1
    if offset != len(blob_result.stdout):
        return None

    records: list[tuple[str, ...]] = [(".", "directory")]
    for relative, mode, object_id in entries:
        if mode == "040000":
            records.append((relative, "directory"))
        elif mode in {"100644", "100755"}:
            records.append((relative, "file", mode, blob_digests[object_id]))
        elif mode == "120000":
            records.append((relative, "symlink", blob_digests[object_id]))
        else:
            return None
    return sorted(records)


def decode_git_checkout_root(raw_output: bytes) -> Path:
    if not raw_output.endswith(b"\n"):
        fail("Git returned a malformed checkout-root record without its LF terminator")
    raw_root = raw_output[:-1]
    if not raw_root or b"\0" in raw_root:
        fail("Git returned an empty or malformed checkout-root record")
    decoded_root = os.fsdecode(raw_root)
    if os.fsencode(decoded_root) != raw_root:
        fail("Git checkout-root bytes could not be decoded losslessly")
    candidate = Path(decoded_root)
    if not candidate.is_absolute():
        fail("Git returned a non-absolute checkout-root record")
    try:
        checkout_root = candidate.resolve(strict=True)
    except (OSError, RuntimeError, ValueError) as error:
        fail(f"Git checkout root cannot be resolved: {safe_text(error)}")
    if not checkout_root.is_dir():
        fail(f"Git checkout root is not a directory: {safe_text(checkout_root)}")
    return checkout_root


def resolve_source_provenance(template_root: Path) -> dict[str, str | None]:
    checkout_root: Path | None = None
    root_result = subprocess.run(
        ["git", "-C", str(template_root), "rev-parse", "--show-toplevel"],
        capture_output=True,
        check=False,
    )
    if root_result.returncode == 0:
        checkout_root = decode_git_checkout_root(root_result.stdout)
        ...
        try:
            relative_template = (
                template_root.resolve(strict=True)
                .relative_to(checkout_root)
                .as_posix()
            )
        except (OSError, RuntimeError, ValueError) as error:
            fail(
                "cannot prove the Guru template belongs to its reported Git checkout: "
                f"{safe_text(error)}"
            )
        if commit_result.returncode == 0 and relative_template:
            ...
            head_records = head_git_tree_records(
                checkout_root,
                relative_template,
                commit,
            )
            apply_record = next(
                (
                    row
                    for row in head_records or []
                    if row[0] == "overlay/apply.sh"
                    and row[1] in {"file", "symlink"}
                ),
                None,
            )
            if (
                # Index equality rejects staged-only state; exact provenance is
                # still proved by comparing the complete live tree with HEAD.
                index_result.returncode == 0
                and apply_record is not None
                and head_records == live_git_tree_records(template_root)
            ):
                return {
                    "kind": "git_commit",
                    "revision": commit,
    ...
    return {
        ...
        "revision": path_digest(template_root, ignore_noise=True),
...
def target_git_root(target: Path) -> Path:
    command = ["git", "-C", str(target), "rev-parse", "--show-toplevel"]
    log(f"run: {command_text(command)}")
    result = subprocess.run(command, capture_output=True, check=False)
    if result.returncode != 0:
        stderr = safe_text(os.fsdecode(result.stderr))
        fail(
            f"command failed with rc={result.returncode}: "
            f"{command_text(command)}: {stderr}"
        )
    return decode_git_checkout_root(result.stdout)


def decode_git_index_path(
    raw_output: bytes,
    relative_to: Path | None = None,
) -> tuple[bytes, Path]:
    if not raw_output.endswith(b"\n"):
        fail("Git returned a malformed index-path record without its LF terminator")
    raw_path = raw_output[:-1]
    if not raw_path or b"\0" in raw_path:
        fail("Git returned an empty or malformed index-path record")
    if not os.path.isabs(raw_path):
        if relative_to is None:
            fail("Git returned a non-absolute index-path record")
        raw_base = os.fsencode(str(relative_to))
        if os.fsdecode(raw_base) != str(relative_to) or not os.path.isabs(raw_base):
            fail("target path bytes cannot anchor the relative Git index path")
        raw_path = os.path.abspath(os.path.join(raw_base, raw_path))
    decoded_path = os.fsdecode(raw_path)
    if os.fsencode(decoded_path) != raw_path:
        fail("Git index-path bytes could not be decoded losslessly")
    index_path = Path(decoded_path)
    if not index_path.is_absolute():
        fail("Git returned a non-absolute index-path record")
    return raw_path, index_path


def read_git_index_state(target: Path) -> tuple[bytes, bytes, bytes]:
    command = [
        "git",
        "-C",
        str(target),
    ...
    raw_path, index_path = decode_git_index_path(result.stdout, target)
    try:
        initial = os.lstat(raw_path)
        ...
        try:
            ...
            fail(f"cannot prove missing Git index state for {index_path}: {safe_text(error)}")
        else:
            fail(f"effective Git index state changed while reading: {index_path}")
    except OSError as error:
        fail(f"cannot inspect effective Git index {index_path}: {safe_text(error)}")
    else:
        if not stat.S_ISREG(initial.st_mode):
            fail(f"effective Git index is not a regular file: {index_path}")
        flags = os.O_RDONLY
        ...
        try:
            ...
            fail(f"cannot open effective Git index {index_path}: {safe_text(error)}")
        try:
            ...
            fail(f"cannot read effective Git index {index_path}: {safe_text(error)}")
        finally:
            os.close(descriptor)
        try:
            final = os.lstat(raw_path)
        except OSError as error:
            fail(f"cannot revalidate effective Git index {index_path}: {safe_text(error)}")
        ...
        if (
            ...
            or fingerprint(initial) != fingerprint(opened)
            or fingerprint(opened) != fingerprint(finished)
            or fingerprint(finished) != fingerprint(final)
        ):
            fail(f"effective Git index changed while reading: {index_path}")
        state = b"file"
        content = b"".join(chunks)
        if len(content) != initial.st_size:
            fail(f"effective Git index size changed while reading: {index_path}")

    return raw_path, state, content


def git_index_query(target: Path, arguments: list[str]) -> bytes:
    command = ["git", "-C", str(target), *arguments]
    result = subprocess.run(
        command,
        capture_output=True,
        check=False,
...
def git_index_record(target: Path) -> bytes:
    raw_path, state, raw_before = read_git_index_state(target)
    records: list[tuple[bytes, bytes]] = []
    if state == b"file":
        queries = (
            (
                b"stage-assume-unchanged",
                ["ls-files", "--stage", "-v", "-z"],
            ),
            (
                b"stage-skip-worktree",
                ["ls-files", "--stage", "-t", "-z"],
            ),
            (
                b"resolve-undo",
        ...
        records = [
            (label, git_index_query(target, arguments))
            for label, arguments in queries
        ]
        after_path, after_state, raw_after = read_git_index_state(target)
        if (after_path, after_state, raw_after) != (raw_path, state, raw_before):
            fail("effective Git index changed while deriving its logical record")

    parts = [
        b"guru-effective-git-index-logical-record-v3\0",
        len(raw_path).to_bytes(8, "big"),
        raw_path,
        len(state).to_bytes(8, "big"),
        state,
    ]
    for label, output in records:
        parts.extend(
            (
                len(label).to_bytes(8, "big"),
                label,
                len(output).to_bytes(8, "big"),
                output,
            )
        )
    return b"".join(parts)
...
def git_index_digest(target: Path) -> str:
    return sha256_bytes(git_index_record(target))
...
def verify_git_index_unchanged(target: Path, expected: str, phase: str) -> None:
    ...
    if actual != expected:
        fail(
            f"{phase} changed the effective Git index logical record: "
...
def create_snapshot(
    ...
    manifest = {
        ...
        "target": str(target),
        "source_commit": (
            source_provenance["revision"]
            if source_provenance["kind"] == "git_commit"
            else None
        ),
        ...
        "git_index_digest_kind": GIT_INDEX_DIGEST_KIND,
        "scopes": scope_rows,
...
def restore_snapshot(target: Path, backup: Path, manifest: dict[str, Any]) -> None:
    ...
    if git_index_digest(target) != manifest["git_index_sha256"]:
        fail("rollback detected a changed effective Git index logical record")
...
def guru_skill_was_managed(
    ...
    template = overlay_dir / "agents-skills" / name
    if paths_equal(source, template, ignore_noise=True):
        return True
    if name not in KNOWN_GURU_SKILL_DIGESTS:
        return True
    records = path_records(source, ignore_noise=True)
    files = [row for row in records if row.get("path") != "."]
    if (
        len(files) == 1
        and files[0].get("path") == "SKILL.md"
        and files[0].get("type") == "file"
        and files[0].get("sha256") in KNOWN_GURU_SKILL_DIGESTS[name]
    ):
        return True
    manifest_path = f"{root}/{name}/SKILL.md"
    return (
        len(files) == 1
        and files[0].get("path") == "SKILL.md"
        and files[0].get("type") == "file"
        and old_hashes.get(manifest_path) == normalized_file_sha256(source / "SKILL.md")
    )
...
def reconcile_skills(
    ...
    for root in (".agents/skills", ".claude/skills"):
        ...
        for child in sorted(old_root.iterdir(), key=lambda item: item.name):
            key = (root, child.name)
            if child.name in guru_owned and guru_skill_was_managed(
                root,
                child,
                child.name,
                old_hashes,
                ...
                continue
            if key in official_owned and child.name not in KNOWN_GURU_SKILL_DIGESTS:
                continue
...
def verify_current_overlay(target: Path, template_root: Path, platform: str) -> None:
    ...
    source_workflow = template_root / "workflows" / workflow_name
    if not source_workflow.is_file() and workflow_name.endswith("-workflow.md"):
        source_workflow = (
            template_root
            / "workflows"
            / f"{workflow_name.removesuffix('-workflow.md')}.md"
        )
    if not source_workflow.is_file():
        fail(f"current checkout workflow is missing: {source_workflow}")
...
def main(argv: list[str]) -> int:
    ...
    source_provenance = resolve_source_provenance(template_root)
    source_checkout_root = source_provenance.get("checkout_root")
    ...
    if source_checkout_root:
        try:
            source_is_target = os.path.samefile(target, source_checkout_root)
        except (OSError, ValueError) as error:
            fail(
                "cannot prove target separation from the Guru source checkout: "
                f"{safe_text(error)}"
            )
        if source_is_target:
    ...
    try:
        ...
        log(
            ...
            f"{source_provenance['kind']}:{source_provenance['revision']}"
        )
        log(f"plan source_checkout_root={source_checkout_root or 'none'}")
        ...
        verify_current_overlay(target, template_root, platform)
        ...
        if git_index_digest(target) != index_before:
            fail("final verification detected a changed effective Git index logical record")
        ...
        completed = True
        log(safe_text(json.dumps(result, ensure_ascii=False, sort_keys=True)))
        ...
        sys.stderr.write(f"REINSTALL_FAILED: {safe_text(error)}\n")
        if mutation_started and not completed and backup is not None and snapshot is not None:
            try:
                ...
                sys.stderr.write(f"ROLLBACK_FAILED: {safe_text(rollback_error)}\n")
                return ROLLBACK_FAILURE_EXIT
            sys.stderr.write(
                "ROLLBACK_OK: original state restored; evidence retained at "
                f"{safe_text(backup)}\n"
            )
        return 1
    finally:
