...
7. **WX-6 存量违例处置**：触碰 SLOT-16 条目时按标准包口径分类记录到 trace §4（绕行须写"为何不修"；顺手修复独立标注；记债给 SLOT-16 编号）。新增代码引入清单外新违例则阻塞（必须修或显式升级人工 Gate）。
