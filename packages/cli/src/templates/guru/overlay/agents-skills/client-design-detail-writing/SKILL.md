...
**破坏性编辑保护**：任何删除、压缩、替换既有详细章节的改动，必须先列 deletion ledger（deleted_category / reason / replacement_location / removes_contract_obligation / reviewer_decision）。接口或后端事实可删除，仍有效的 L1 章节骨架、UNIT/BHV 承接、行为列表、输入输出错误合同、状态 owner、失败收口、测试映射、不得补造清单必须保留、移到命名替代位置，或以 `N/A：<理由>` 显式声明；不得把行为合同压扁成 endpoint/interface 映射表。
...
audit；章节写作批次不是实现切片边界。Flutter 的
domain→data→presentation→横切归属与依赖律继续生效，但不得机械要求每个
`UNIT`、`doc_type`、层、ViewModel、UseCase 或 View 各占一个切片。
