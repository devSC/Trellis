...
- **vs 设计**审**（人工 Gate）**：写完后由配套的 `h5-design-overview-review` skill 做**人工 Gate 判定**——「这份概要能否进入下一阶段（详细设计）」，判的是归属是否正确、是否违反分层依赖律（server-first / 'use client' 最小化 / 私有数据获取边界）、架构视图是否就绪、承接索引是否闭合。这是结构性/语义性判定。
