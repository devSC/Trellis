...
版本化场景可在 `versions/` 下按同一结构分目录维护（详见下文「版本与变更管理（版本化需求包）」节）。
...
版本化需求包的目录结构、`manifest.yaml`、`changes/` 变更承载、`traceability`/`decisions`/`snapshots` 治理层与版本演进规则，**主定义在标准包 `requirement-structure-single-source.md` §15「版本化需求包」**；本节不复写规则正文，只补 `requirement-writing` 阶段的操作要点（规则口径以 §15 为准）。

执行方式（版本化场景）：
1. 按 §15.1 在 `versions/<version>/` 建版本目录，内部章节承载、编号契约、核心能力定义沿用标准包 §2-§6（与扁平结构一致，不另造并行体系）。
2. 写 `manifest.yaml`（§15.2），更新 README 版本入口 / 版本矩阵。
3. 正式需求变更在 `changes/change-log.md` 增汇总条目（§15.3），详细入 `changes/changes/`；**禁日期子目录、不另造 `changelog.md`**。
4. 每轮「完成收敛」作为独立变更记录，并过 §15.8 版本化完成收敛附加项。
5. 跨版本演进默认 full-copy + diff（§15.7）。

非版本化场景：扁平 `requirements/`（见上「模块化文档结构」）仍是默认合法形态。
