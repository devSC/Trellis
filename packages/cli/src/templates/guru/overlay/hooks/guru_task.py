...
def _gate_digest(task_data: dict, task_dir: Path) -> str:
    ...
    contract_data = _read_json(contract)
    if (
        contract_data.get("policy_version") == guru_contract.POLICY_VERSION_V2
        and guru_contract.contract_route(contract_data) == guru_contract.ROUTE_FULL_CHAIN
    ):
        return _digest({
            "schema_version": 2,
            "guru_chain": task_data.get("guru_chain"),
            "require_req_uc": task_data.get("require_req_uc"),
            "review_runs": (task_data.get("guru_gates") or {}).get("review_runs"),
