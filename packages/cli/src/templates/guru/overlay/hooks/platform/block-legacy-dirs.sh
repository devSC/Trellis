#!/usr/bin/env bash
# PreToolUse：拦截在老目录新建文件（golden-path §10 / 项目约定的存量/老目录槽位）。双端通用脚本。
...
# 安装时按目标项目 project-conventions 的存量/老目录槽位 填写 LEGACY_PATTERNS（grep -E 语法，| 分隔）。
...
LEGACY_PATTERNS=''
# 留空 = 该项目无老目录（无老目录时可显式留空，合规），拦截按设计不生效；非空才按 grep -E 拦截。
...
if [ -n "$HIT" ]; then
  echo "BLOCKED: $HIT 位于项目约定禁止新建的老目录。新代码请进 SLOT-02 的 canonical 目录；确属维护存量文件请用户确认后操作。" >&2
  exit 2
fi
exit 0
