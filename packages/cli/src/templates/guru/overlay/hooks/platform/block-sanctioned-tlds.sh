...
INPUT=$(cat)
# host 段用 [^/"?# ]* 锚定（不跨第一个 / ? 或 #，只看域名 authority、不进 path/query/fragment——避免误拦
# path/fragment 里的 .cu/.sy 文件名如 github.com/x/kernel.cu 或 example.com#kernel.cu）；边界 [^a-z0-9.-]
# 覆盖 markdown )、逗号、] 等 host 侧常见收尾；grep -i 大小写不敏感（DNS 域名 .IR/.Ir 等价可解析）。
