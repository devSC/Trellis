...
- `medium -> lite_task`：局部业务行为、单层逻辑、小范围多文件或需要可复跑验证的变更，进入 lite route（产物形态仍复用既有 `guru_chain=light`）。
