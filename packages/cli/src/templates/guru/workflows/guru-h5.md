...
无需求→1.1；无概要→1.3；无详细→1.4（full=设计包，light=design.md）。每步=writing→review skill→guru_gate.py confirm 人工收口（通道按 gate_mode）；三确认齐才 task.py start。doc_type 限七类，归属违分层依赖律=fail；client 直取私有数据=fail。
[/workflow-state:planning]
...
实现→质检→spec回写→commit→finish。dispatch trellis-implement/check，prompt 以 Active task: <path> 开头；trace 记执行/tsc+next build+eslint 证据/偏差；质检按 guru H5 口径（分层律+server-client 边界+doc_type 七类）；无 build/lint 证据不 commit；设计缺陷回 Phase1。
[/workflow-state:in_progress]
...
**概要 Gate**：加载 `h5-design-overview-review` 审核，结论"可进入详细设计"后，完成 **confirm 人工收口**（通道按 gate_mode）；确认落盘后方可进 1.4。**归属违反分层依赖律 / client 直取私有数据 = 直接 fail**（不放行、回本步重排归属）。
...
**详细 Gate**：加载 `h5-design-detail-review` 审核，结论"可进入编码"后，完成 **confirm 人工收口**（通道按 gate_mode）；确认落盘后方可进 1.5/1.6。
...
| 概要主定义过概要 Gate + 用户 confirm overview（full=design-main.md；light=design.md §1；归属表无分层律违规、无 client 直取私有数据） | ✅ |
| 详细设计过详细 Gate + 用户 confirm detail（full=chapters/ 闭合；light=design.md §2；doc_type 限七类，pending 类已豁免或补 L2） | ✅ |
